...
void main(){
    ...
    if (uIsLight > 0.001) {
        vec3 lightCol = vec3(0.98, 0.98, 0.99) - col.rgb * 0.85;
        col.rgb = mix(col.rgb, clamp(lightCol, 0.0, 1.0), uIsLight);
    }
...
const loop = () => {
  ...
  const targetLight = isLightMode.value ? 1.0 : 0.0;
  currentIsLight += (targetLight - currentIsLight) * 0.08;
  ...
  program.uniforms.uIsLight.value = currentIsLight;
...
onMounted(() => {
  ...
  currentIsLight = isLightMode.value ? 1.0 : 0.0;
